...
const updateAvatarController = async (req, res, next) => {
    await User.findOneAndUpdate({ userEmailID: req.body.userEmailID }, req.body, {
        ...
        res.status(200).json({
            code: 200,
            message: response ? "Avatar Updated Successfully" : "User not found or the email does not exists"
        })
    }).catch((err) => {
        res.json(err);
    })
...
module.exports = { registerController, loginController, updateAvatarController }    